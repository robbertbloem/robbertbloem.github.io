...
def write_header(f):
    ...
    f.write("</head>")
    f.write("<body>")
    f.write("<h1>Matplotlib examples and reference</h1><p>After using Matplotlib for some time I saw that I had to look up the same things over and over again. 'How did I move the labels away from the axes?' The examples on the Matplotlib website are useful but too often too complicated. Different concepts are combined in a single example, leaving the user (me) to figure out how the example works, instead of finding a solution to my problem. </p><p>I started these pages as a reference for myself, but quickly realized that other people might be interested as well. It does however mean that the examples mostly treat what I use myself.</p><p>The examples in 'Basics' and 'Line properties' are enough to quick-and-dirty plot some data. The examples after that show further customizations to make 'publication quality' plots. Examples 002 and 020 deserve some special attention. Example 002 explains the use of rcParams to change properties globally. Example 020 introduces the hierarchy of Matplotlib with Figures, Axes and Artists. It adds complexity but also flexibility and power to Matplotlib.</p>")
